...
namespace TwinKleS.Entry {
				...
				if (!option.must_exist) {
					...
					if (!test_function(result)) {
						...
						while (true) {
							let input = Input.string(null)!;
							if (input === ':s') {
								if (CoreX.System.name !== 'windows') {
									throw new MyError(`:s only enabled on windows system`);
								}
								...
								let selected = CoreX.System.Windows.select_file_by_dialog(pick_folder, false);
	...
	export function _injector(
		...
		switch (CoreX.System.name) {
			case 'windows': {
				let output_mode = CoreX.System.Windows.get_standard_console_mode('output');
				if ((output_mode & CoreX.System.Windows.OutputConsoleMode.ENABLE_VIRTUAL_TERMINAL_PROCESSING) !== 0) {
					Output.g_enable_virtual_terminal_sequences = true;
				} else {
					try {
						CoreX.System.Windows.set_standard_console_mode('output', output_mode | CoreX.System.Windows.OutputConsoleMode.ENABLE_VIRTUAL_TERMINAL_PROCESSING);
						Output.g_enable_virtual_terminal_sequences = true;
					} catch (e) {
		...
		CoreX.FileSystem.change_directory(g_workspace);
