/**
 * + data.base64.encode Base64 编码
 * + data.base64.decode Base64 解码
 */
namespace TwinKleS.Entry.method.data.base64 {
				...
				worker(a: Entry.CFSA & {
					...
					let ripe_file: string;
					{
						...
						ripe_file = ArgumentParser.path(a.ripe_file, {
							...
							default_value: raw_file.replace(/()?$/i, '.base64_ripe.txt'),
							must_exist: false,
				...
				worker(a: Entry.CFSA & {
					...
					let raw_file: string;
					{
						...
						raw_file = ArgumentParser.path(a.raw_file, {
							input_message: '请输入输出路径',
							default_value: ripe_file.replace(/()?$/i, '.base64_raw.bin'),
							must_exist: false,
							if_exist: a.fs_if_exist,
						});
					}
...
({
	injector: TwinKleS.Entry.method.data.base64._injector,
});