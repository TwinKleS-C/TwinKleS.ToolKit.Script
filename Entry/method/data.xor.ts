/**
 * + data.xor.crypt XOR 加解密
 */
namespace TwinKleS.Entry.method.data.xor {
				...
				worker(a: Entry.CFSA & {
					...
					let key: bigint;
					{
						...
						cipher_file = ArgumentParser.path(a.cipher_file, {
							...
							default_value: plain_file.replace(/()?$/i, '.xor.bin'),
							must_exist: false,
...
({
	injector: TwinKleS.Entry.method.data.xor._injector,
});