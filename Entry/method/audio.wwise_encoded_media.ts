/**
 * + audio.wwise_encoded_media.decode Wwise-Encoded-Media 解码
 * + audio.wwise_encoded_media.decode.batch [批处理] Wwise-Encoded-Media 解码
 */
namespace TwinKleS.Entry.method.audio.wwise_encoded_media {
	...
	export function _injector(
		...
		g_executor_method.push(
			Executor.method_of({
				id: 'audio.wwise_encoded_media.decode',
				...
				worker(a: Entry.CFSA & {
					...
					CoreX.Tool.Audio.WwiseEncodedMedia.decode_fs(ripe_file, raw_file, Main.path_at_home(config.tool.ffmpeg), Main.path_at_home(config.tool.ww2ogg), Main.path_at_home(config.tool.ww2ogg_pcb), Main.path_at_home(config.temp_directpry));
		...
		g_executor_method_of_batch.push(
			Executor.method_of({
				id: 'audio.wwise_encoded_media.decode.batch',
...
({
	injector: TwinKleS.Entry.method.audio.wwise_encoded_media._injector,
});